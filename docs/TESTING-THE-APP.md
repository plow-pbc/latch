...
| Owner-facing | any confirmed block above | the tray gains a "Needs …" item; a notification carries the owner sentence; both land on the Capabilities tab; the Audit row's Status reads "Blocked · <permission>" in amber beside a green Allowed decision, and the Status filter's `Blocked` shows it |
