...
| Setup: Password import handoff | finish Availability, then choose `Import passwords` on the final page | three overlapping agent marks and the task stay above the horizontal Gatekeeper beam; Browser Vault and the site stay below it; the only line carrying a password runs from Vault to the browser. Setup closes, the main window opens directly on Browser Vault, and its existing Import sheet opens automatically; Apple Passwords, 1Password, Chrome and CSV still use the same staging, preview and commit path as the Vault tab's Import button. If Browser use was switched off, the action explicitly reads `Enable Browser & import passwords` and enables it before opening that same sheet |
