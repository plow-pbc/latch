...
| Owner-facing | any confirmed block above | the tray gains a "Needs …" item; a notification carries the owner sentence; both land on Settings, where the Capabilities card shows every row of the inventory; the Audit row wears the amber badge with the permission named |
| Setup | fresh home, sign in | the "Data & permissions" step's Full Disk Access request opens the grant flow, and its status turns granted after the drag without reopening setup (it reads the same `capabilities:get` snapshot as Settings) |
