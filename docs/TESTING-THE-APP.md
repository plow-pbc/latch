...
| Owner-facing | any confirmed block above | the tray gains a "Needs …" item; a notification carries the owner sentence; both land on the Capabilities tab; the Audit row wears the amber badge with the permission named and the `Blocked` chip filters to it |
...
| Capabilities: the buttons | each row in turn | Full Disk Access and Accessibility float the drag panel beside their pane; Contacts and Calendars "Ask macOS now" raise the system dialog (packaged build — from source macOS refuses without the usage strings) and, once refused, "Open System Settings…" floats the pointer panel; a folder's "Ask macOS now" raises its dialog; an Automation row's "Grant…" opens the target app and raises its dialog, and a refused pair's button opens the Automation pane with the panel beside it |
