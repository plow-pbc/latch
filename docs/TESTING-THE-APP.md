...
| Owner-facing | any confirmed block above | the tray gains a "Needs …" item; a notification carries the owner sentence; both land on the Permissions section when the block names a switch, and on the Audit tab's Blocked view when it does not (a locked file, SIP, POSIX permissions); the item comes down on its own when the owner answers the dialog and the run goes on; the Audit row's Status reads "Blocked · <permission>" in amber beside a green Allowed decision, and the Status filter's `Blocked` shows it |
