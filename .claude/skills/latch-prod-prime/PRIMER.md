...
- `device/approvals/*.json` — approvals **in flight** only: a record is written before the decision is made — and BEFORE this Mac knows who will make it, so an in-flight record may have no actor at all — then removed once that decision is in the audit log. Who eventually decides is the approval mode (`routeIntent` in `apps/desktop/src/reviewPolicy.ts`): the owner at a dialog, the AI reviewer, or a standing policy that allows or denies with neither of them involved. Read `source` on the settled record, not the record's existence, to learn which, so an empty directory is the normal state. Fields: `intentId`, `agentId`, `agentName`, `capabilities[]`, `status` (`pending`, or settled with `decision`/`source`/`decidedAt` and `recorded: false` for the instant before the audit append). Anything here after a clean start is a bug, not history — the audit log is the history.
...
Decisions live in the audit log, paired to the request by `intentId`; the approvals directory holds only decisions still in flight — including ones no actor has been chosen for yet:
...
# Decisions not yet made — by whoever this Mac's mode elects (normally none)
