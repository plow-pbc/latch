...
- `app/settings.json` — `relayCredentialEnc` (**SECRET — never print**; the active login session sealed with the OS keychain) or `relayCredential` (**SECRET — never print**; its plaintext 0600 floor). `pendingRevokeCredentialsEnc` and `pendingRevokeCredentials` are likewise **SECRET**: sessions forgotten locally but awaiting confirmed server retirement. Inspect only presence/count/length, never values. Also `accountUid`, `mcpUrl`, `approvalMode` (e.g. `adversarial`), `agentPurpose` (free text), `setupComplete`, `telemetryEnabled`, `autoCheckUpdates`, `windowBounds.*`.
...
```bash
# settings.json — presence/count/length only for credentials
jq -r '{approvalMode, mcpHost: (.mcpUrl|sub("^https?://";"")|split("/")[0]), signedIn: (((.relayCredentialEnc // "") + (.relayCredential // "")) | length > 0), relayCredLen: (((.relayCredentialEnc // "") | length) + ((.relayCredential // "") | length)), pendingRevokeCount: (((.pendingRevokeCredentialsEnc // []) | length) + ((.pendingRevokeCredentials // []) | length)), accountUidLast3: (.accountUid|tostring|.[-3:])}' "$HOME/Library/Application Support/Plow-Latch/app/settings.json"
