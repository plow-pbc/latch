...
- `fixtures/*.json` golden vectors, asserted byte-for-byte by the TS suite.
  Six remain: canonical JSON, identity, intent, path canonicalization, rule
  keys, SBPL. The grant's signing bytes ride in `intent.json`, which was
  re-frozen without the intent's own signature; the key-schedule, AEAD-frame
  and connection-string vectors went with the concepts they froze. Generated by
  the Swift implementation during the migration; now frozen (the generator is
  gone).
