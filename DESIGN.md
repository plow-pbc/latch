...
- `file-write*` stays scoped: the approved write paths + the scratch dir + a
  ...
  writes don't break tools — except a run that may be reaped for going
  silent, which gets none of them (`SANDBOX-BOUNDARY.md` §1). Writes to arbitrary or system locations are denied —
