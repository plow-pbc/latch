...
describe("parseAgentIndex", () => {
  it.each([
    ["a deployable entry", [LIFE], { life: { blurb: "Runs a household.", builder: "Sam", users: 16, successRate: 88 } }],
    ...
    ["a malformed entry, dropped without the others", [null, 7, { name: "no id" }, LIFE], {
      life: { blurb: "Runs a household.", builder: "Sam", users: 16, successRate: 88 },
    }],
    ...
    }], { odd: { blurb: null, builder: null, users: 0, successRate: null } }],
    ["an id that is an Object prototype key, kept as data", [{ ...LIFE, agent_id: "__proto__" }], {
      ["__proto__"]: { blurb: "Runs a household.", builder: "Sam", users: 16, successRate: 88 },
    }],
...
describe("fetchAgentIndex", () => {
  it("reads the public list without any credential", async () => {
    ...
    expect(index.life?.users).toBe(16);
  });
