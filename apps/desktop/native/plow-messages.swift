...
/// A JSON value, ordered.
///
/// `JSONSerialization` sorts or hashes keys, and the documented row shape names
/// its keys in an order a human reads down. A dictionary cannot promise that,
/// so rows are built as ordered pairs and encoded here.
enum JSON {
    case string(String)
    case int(Int64)
    case bool(Bool)
    ...
    /// RFC 8259 string escaping, including the C0 controls a message body can
    /// genuinely contain (a NUL survives the typedstream decode of some rows).
...
/// Print one row as a single line of JSON, keys in the order given.
func emit(_ pairs: [(String, JSON)]) {
    let body = pairs.map { "\(JSON.quote($0.0)):\($0.1.encoded)" }.joined(separator: ",")
    print("{\(body)}")
}
...
struct Message {
    ...
    func write() {
        emit([
            ("rowid", .int(rowid)),
            ("chat_guid", chatGuid.map(JSON.string) ?? .null),
            ("chat_identifier", chatIdentifier.map(JSON.string) ?? .null),
            ("display_name", displayName.map(JSON.string) ?? .null),
            ("sender", sender.map(JSON.string) ?? .null),
            ("is_from_me", .bool(isFromMe)),
            ("at", .string(isoOut.string(from: Date(timeIntervalSince1970: at)))),
            ("body", body.map(JSON.string) ?? .null),
        ])
    }
}
...
func runChats(_ o: Options, _ store: Store) {
    ...
    store.query(sql, []) { r in
        ...
        emit([
            ("chat_id", .int(r.int(0))),
            ("guid", r.string(1).map(JSON.string) ?? .null),
            ("chat_identifier", r.string(2).map(JSON.string) ?? .null),
            ("display_name", r.string(3).map(JSON.string) ?? .null),
            ("kind", r.string(5).map(JSON.string) ?? .null),
            ("last_message", .string(isoOut.string(from: Date(timeIntervalSince1970: at)))),
        ])
    }
}
