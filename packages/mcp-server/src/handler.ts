...
Whenever a call returns status 'denied', tell the user to open Plow Latch on their Mac to review what Gatekeeper or the owner denied and see the available next steps. Relay the result's reason; it distinguishes a human choice from a condition the user can fix. Do not retry unless the reason explicitly says a retry is available, and then wait for the user to act first.
