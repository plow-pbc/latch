...
describe("BrowserHost", () => {
  ...
  it("answers the exit itself when the browser dies before saying hello", async () => {
    ...
    // that would be asserting a race; making it reliable is
    // plow-pbc/domo-desktop#98.
    await expect(host.ensureReady()).rejects.toThrow(/exited \(code=9/);
  });

  it("answers a spawn that never happened, twice in a row", async () => {
    // ENOENT on the browser binary: `error` fires and `exit` never does, so
    // this is the one settle path that has to let go on its own. What the test
