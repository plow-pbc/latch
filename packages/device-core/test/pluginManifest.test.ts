...
describe("parseManifest", () => {
  ...
  it.each([
    ...
    ["a binary without a sha for every arch", withPatch({ runtime: { binaries: [{ name: "b", url: { arm64: "https://x/b", x64: "https://x/b" }, sha256: { arm64: "a".repeat(64) } }], sources: [] } }), "binary b needs a sha256 for arm64 and x64"],
    ...
    ["a permission id that is not a macOS switch", withPatch({ requires: { permissions: ["telepathy"] } }), "requires.permissions entries must name a macOS permission this Mac has a switch for"],
