...
const m = parseManifest(JSON.stringify({ ...MINIMAL, env: {
  ...
  A_OWNER: { fixed: "${owner_home}/Plow/wiki" },
}}));
const ctx = { pluginHome: "/p/home", ownerHome: "/o/home", port: 4242, plowApiBase: "https://api.example", secret: (n: string) => `S:${n}`, mint: async (s: string) => `M:${s}` };

describe("resolveEnv", () => {
  it("substitutes every placeholder and reads each source", async () => {
    await expect(resolveEnv(m, ctx)).resolves.toEqual({
      A_HOME: "/p/home", A_URL: "http://127.0.0.1:4242", A_BASE: "https://api.example/v1", A_TOKEN: "S:daemon-token", A_KEY: "M:llm:chat",
      A_OWNER: "/o/home/Plow/wiki",
    });
  });
