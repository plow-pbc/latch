...
export interface PluginManifest {
  ...
  runtime: {
    // A pinned downloaded archive, verified by sha256 per-arch.
    binaries: { name: string; url: Record<"arm64" | "x64", string>; sha256: Record<"arm64" | "x64", string>; executable?: string }[];
    // A git repo cloned at a pinned commit (the commit hash IS the integrity
    // pin — no separate digest applies), then built by its own `install`
    // argv. Staged once per arch, same as a binary.
