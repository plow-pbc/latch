...
export class DeviceAgent {
   ...
   *  plugin's fixed env. Empty until then. */
  private plowApiBase = "";
  ...
  /**
   * Load installed plugins and register their skills. Called once after
  ...
  private async executeCommand(
    ...
    // (fixed or secret — a `mint` source has no implementation in this
    // build), never the provider orchestration below.
   ...
   * and daemon-health refusal run BEFORE any env is resolved, so an
  ...
  private async executePlugin(
    ...
    try {
      env = await this.plugins.env(plugin, null, this.plowApiBase);
    } catch (e) {
      const message = e instanceof PluginError ? e.message : `could not authorise ${plugin.manifest.command}`;
      return this.execError(intent.intentId, message);
    }
    this.audit.record("exec_start", { intentId: intent.intentId, argv });
    try {
      // manifest.ts validates exec.cwd to be "plugin" (the cloned repo) or a
      // declared source name, and this build refuses every manifest that
      // declares a source (runtime.sources) — so "plugin" is the only cwd a
      // plugin installed here can ever have. `cwd` is always added to the
      // sandbox's reads (Executor.run), which is what makes the plugin's own
      // tree — and never the owner's approved paths — readable for this run.
      const cwd = plugin.dirs.repo;
      const result = await this.executor.run({
        argv: [...plugin.manifest.exec.argv, ...argv.slice(1)],
        cwd,
