...
- **A credential never goes in a URL, a log line, an error string, or the audit
  ...
  stale. `settings.json` holds it and is written
  `0600`, encrypted at rest wherever the OS offers a way and in the clear where
  it does not — the file permission is the floor that always holds, and the
  encryption is what is added on top of it, never instead. Describe it that way
  and leave the mechanism to the code. The renderer is never given it.
